  ...
  <div class="space-y-10">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 relative h-[280px] rounded-[2.5rem] bg-gray-900 overflow-hidden shadow-2xl shadow-primary/20 group">
        ...
        <div class="relative h-full p-10 flex flex-col justify-between">
          <div class="flex items-center justify-between">
            ...
            <div class="px-4 py-1.5 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-sm  uppercase  flex items-center">
          ...
          <div>
             <p class="text-sm font-bold text-gray-400 uppercase  mb-3">Available Balance</p>
             <div class="flex items-baseline space-x-4">
                <h3 class="text-5xl  text-white er">{{ formatPrice(walletBalance) }}</h3>
                <span class="text-sm font-bold text-gray-400 uppercase ">{{ currentCurrency.code }}</span>
             </div>
          </div>

          <div class="flex items-center space-x-10">
            <div>
               <p class="text-sm font-bold text-gray-400 uppercase  mb-1">Account Number</p>
               <p class="text-base  text-white ">•••• •••• 4242</p>
            </div>
            <div class="h-10 w-px bg-white/10"></div>
            <div>
               <p class="text-sm font-bold text-gray-400 uppercase  mb-1">Agency Tier</p>
               <p class="text-base  text-white uppercase ">Premium Agent</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-[2.5rem] border border-gray-100 p-8 shadow-sm flex flex-col justify-between">
        <h4 class="text-sm  text-gray-900 uppercase  mb-6">Quick Actions</h4>
        <div class="space-y-4">
          <button @click="showTopUpModal = true" class="w-full h-16 bg-primary text-white rounded-2xl  uppercase  flex items-center justify-center group hover:shadow-xl hover:shadow-primary/20 transition-all transition-all duration-300">
            <PlusIcon class="h-5 w-5 mr-3 group-hover:rotate-90 transition-transform" />
            Top Up Wallet
          </button>
          <button class="w-full h-16 bg-gray-50 text-gray-800 rounded-2xl  uppercase  border border-gray-100 hover:bg-gray-100 transition-all">
            Transfer Funds
          </button>
        </div>
        <p class="text-sm text-gray-400 font-bold uppercase  text-center mt-6">Instant settlements via Stripe</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
       <div v-for="stat in financeStatsList" :key="stat.label" class="bg-white p-8 rounded-[2rem] border border-gray-100 group hover:border-primary/20 transition-all">
          <div class="flex items-center justify-between mb-4">
             <p class="text-sm font-bold text-gray-400 uppercase ">{{ stat.label }}</p>
          ...
          <div class="flex items-baseline space-x-2">
             <p class="text-2xl  text-gray-900 ">{{ formatPrice(stat.value) }}</p>
             <span v-if="stat.trend" class="text-sm  text-emerald-500 uppercase er">{{ stat.trend }}</span>
          </div>
       </div>
    </div>
    ...
    <div class="bg-white border border-gray-100 rounded-[2.5rem] overflow-hidden shadow-sm shadow-gray-100/10">
       <div class="p-8 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div class="flex items-center space-x-8">
             <button v-for="tab in ['Wallet History', 'Invoices', 'Payouts']" :key="tab" @click="activeTab = tab" class="text-sm  uppercase  transition-all relative pb-2" :class="activeTab === tab ? 'text-primary' : 'text-gray-400 hover:text-gray-600'">
               {{ tab }}
             ...
             <table class="w-full text-left">
                <thead>
                   <tr class="bg-gray-50/50">
                      <th class="px-8 py-5 text-sm font-bold uppercase  text-gray-400">Transaction ID</th>
                      <th class="px-8 py-5 text-sm font-bold uppercase  text-gray-400">Type</th>
                      <th class="px-8 py-5 text-sm font-bold uppercase  text-gray-400">Description</th>
                      <th class="px-8 py-5 text-sm font-bold uppercase  text-gray-400">Amount</th>
                      <th class="px-8 py-5 text-sm font-bold uppercase  text-gray-400">Status</th>
                      <th class="px-8 py-5 text-sm font-bold uppercase  text-gray-400">Date</th>
                   </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                   ...
                   <tr v-else-if="transactions.length === 0" class="text-center py-20">
                      <td colspan="6" class="py-32 text-sm font-bold text-gray-500 uppercase  opacity-40">No wallet activity recorded yet</td>
                   </tr>
                   <tr v-else v-for="tx in transactions" :key="tx._id" class="hover:bg-gray-50/50 transition-colors group">
                      <td class="px-8 py-6">
                         <span class="text-sm  text-gray-900 ">{{ tx.reference }}</span>
                      </td>
                      <td class="px-8 py-6">
                      ...
                      <td class="px-8 py-6">
                         <p class="text-sm  " :class="tx.type === 'debit' ? 'text-rose-600' : 'text-emerald-600'">
                           {{ tx.type === 'debit' ? '-' : '+' }}{{ formatPrice(tx.amount) }}
                         </p>
                      </td>
                      <td class="px-8 py-6">
                         <span class="text-sm  px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full uppercase er border border-emerald-100">{{ tx.status }}</span>
                      </td>
                      <td class="px-8 py-6">
    ...
    <BaseModal :show="showTopUpModal" title="Top Up Wallet" @close="showTopUpModal = false">
       <div class="p-6 space-y-8">
          <div class="space-y-4">
             <label class="text-sm  text-gray-900 uppercase ">Amount to Fund</label>
