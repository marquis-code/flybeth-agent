<template>
  <div class="container mx-auto space-y-8">
    ...
    <div class="border-b border-gray-100 pb-4">
      ...
      <p class="text-sm text-gray-400 font-bold  ">Manage your agent profile, security, and preferences</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      <div class="space-y-1">
        <button 
          ...
          class="w-full flex items-center px-4 py-2.5 rounded-lg text-sm font-bold transition-all text-left"
      ...
      <div class="md:col-span-2 space-y-8">
        ...
        <div v-if="activeSection === 'profile'" class="space-y-6">
           <div class="flex items-center gap-6 p-6 bg-white border border-gray-100 rounded-xl relative overflow-hidden group">
              ...
              <div>
                 ...
                 <span class="inline-block mt-2 px-2 py-0.5 bg-emerald-50 text-emerald-600 text-sm font-bold rounded border border-emerald-100 uppercase">Verified Agent</span>
              </div>
              <div v-if="localLoading" class="absolute inset-0 bg-white/60 backdrop-blur-[2px] flex items-center justify-center">
                 <div class="flex flex-col items-center">
                    ...
                    <span class="mt-2 text-sm font-bold text-primary uppercase">Synchronizing...</span>
                 </div>
              </div>
           </div>
           ...
           <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1.5">
                 ...
                 <input v-model="profileForm.firstName" type="text" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-gray-900 outline-none focus:border-primary transition-colors" />
              </div>
              <div class="space-y-1.5">
                 <label class="text-sm font-bold text-gray-400 px-1">Last Name</label>
                 <input v-model="profileForm.lastName" type="text" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-gray-900 outline-none focus:border-primary transition-colors" />
              </div>
           </div>
           
           <div class="space-y-1.5">
              <label class="text-sm font-bold text-gray-400 px-1">Phone Number</label>
              <input v-model="profileForm.phone" type="tel" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-gray-900 outline-none focus:border-primary transition-colors" />
           </div>

           <div class="space-y-1.5">
              <label class="text-sm font-bold text-gray-400 px-1">Email Address</label>
              <input :value="user?.email" type="email" disabled class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm font-bold text-gray-400 outline-none cursor-not-allowed" />
           </div>

           <div class="pt-2">
              <button class="px-6 py-2.5 bg-primary-dark text-white text-sm font-bold rounded-lg hover:bg-black transition-colors" @click="handleUpdateProfile">
        ...
        <div v-if="activeSection === 'agency'" class="space-y-6">
           <div class="p-6 bg-white border border-gray-100 rounded-xl space-y-6">
              <h4 class="text-sm font-bold text-gray-900 border-b border-gray-50 pb-4">Business Information</h4>
              
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div class="space-y-1.5 md:col-span-2">
                    <label class="text-sm font-bold text-gray-400 px-1">Agency / Company Name</label>
                    <input v-model="agencyForm.agencyName" type="text" placeholder="e.g. Dream Travels Ltd." class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-gray-900 outline-none focus:border-primary transition-colors" />
                 </div>
                 <div class="space-y-1.5">
                    <label class="text-sm font-bold text-gray-400 px-1">Registration Number</label>
                    <input v-model="agencyForm.registrationNumber" type="text" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-gray-900 outline-none focus:border-primary transition-colors" />
                 </div>
                 <div class="space-y-1.5">
                    <label class="text-sm font-bold text-gray-400 px-1">Country of Operation</label>
                    <input v-model="agencyForm.country" type="text" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-gray-900 outline-none focus:border-primary transition-colors" />
                 </div>
                 <div class="space-y-1.5">
                    <label class="text-sm font-bold text-gray-400 px-1">Business Website</label>
                    <input v-model="agencyForm.website" type="url" placeholder="https://" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-gray-900 outline-none focus:border-primary transition-colors" />
                 </div>
                 <div class="space-y-1.5">
                    <label class="text-sm font-bold text-gray-400 px-1">WhatsApp Business Number</label>
                    <input v-model="agencyForm.whatsappNumber" type="tel" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-gray-900 outline-none focus:border-primary transition-colors" />
                 </div>
                 <div class="space-y-1.5 md:col-span-2">
                    <label class="text-sm font-bold text-gray-400 px-1">Official Business Address</label>
                    <textarea v-model="agencyForm.businessAddress" rows="2" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-gray-900 outline-none focus:border-primary transition-colors"></textarea>
                 </div>
              </div>
              
              <div class="pt-2">
                 <button class="px-6 py-2.5 bg-primary-dark text-white text-sm font-bold rounded-lg hover:bg-black transition-colors" @click="handleUpdateAgency">
        ...
        <div v-if="activeSection === 'security'" class="space-y-6">
           <div class="p-6 bg-white border border-gray-100 rounded-xl space-y-4">
              <div class="flex items-center justify-between">
                 <h4 class="text-sm font-bold text-gray-900">Multi-Factor Authentication</h4>
                 <div 
                   @click="toggle2FA"
              ...
              <p class="text-sm text-gray-500 leading-relaxed font-medium">Protect your account with an additional layer of security. We'll send a code to your mobile device whenever you log in from an unrecognized device.</p>
           </div>

           <div class="space-y-4 pt-4">
              <h4 class="text-sm font-bold text-gray-900 border-b border-gray-50 pb-2">Active Sessions</h4>
              <div v-for="(session, idx) in dummySessions" :key="idx" class="flex items-center justify-between p-4 bg-white rounded-lg border border-gray-100">
                 <div class="flex items-center gap-3">
                    ...
                    <div class="flex flex-col">
                       <span class="text-sm font-bold text-gray-800">{{ session.browser }} on {{ session.os }} • {{ session.location }}</span>
                       <span class="text-sm text-gray-400">Last active: {{ session.lastActive }}</span>
                    </div>
                 </div>
                 <span v-if="session.isCurrent" class="text-sm  text-emerald-500">Current</span>
                 <button v-else class="text-sm font-bold text-rose-500 hover:underline">Revoke</button>
              </div>
           </div>
        </div>
        ...
        <div v-if="activeSection === 'notifications'" class="space-y-6">
           <div class="p-6 bg-white border border-gray-100 rounded-xl space-y-6">
              <h4 class="text-sm font-bold text-gray-900">Communication Channels</h4>
              <div class="space-y-4">
                 <div v-for="channel in channels" :key="channel.key" class="flex items-center justify-between">
                    <div>
                       <h5 class="text-sm font-bold text-gray-800">{{ channel.label }}</h5>
                       <p class="text-sm text-gray-400 font-medium">{{ channel.description }}</p>
                    </div>
                    <div 
           ...
           <div class="p-6 bg-white border border-gray-100 rounded-xl space-y-4">
              <h4 class="text-sm font-bold text-gray-900">Alert Categories</h4>
              <p class="text-sm text-gray-500 leading-relaxed font-medium">Select what activities you want to be notified about.</p>
        ...
        <div v-if="activeSection === 'regional'" class="space-y-6">
           <div class="p-6 bg-white border border-gray-100 rounded-xl space-y-6">
              ...
              <div class="pt-2">
                 <button class="px-6 py-2.5 bg-primary-dark text-white text-sm font-bold rounded-lg hover:bg-black transition-colors" @click="handleUpdateRegional">
