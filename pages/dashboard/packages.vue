  ...
  <div class="space-y-6">
    ...
    <div class="space-y-4">
       ...
       <div v-else-if="packagesList.length === 0" class="bg-white border-2 border-dashed border-gray-100 rounded-3xl p-24 text-center">
          ...
          <p class="text-sm text-gray-400 font-medium max-w-xs mx-auto mb-8">Construct a curated bundle to share with your clients and display on your storefront.</p>
          <button @click="startNewOffer" class="px-8 py-3 bg-primary text-white text-sm font-bold rounded-xl shadow-lg">Construct Bundle</button>
       </div>
          ...
          <div v-for="pkg in packagesList" :key="pkg._id" class="bg-white rounded-3xl border border-gray-100 overflow-hidden hover:shadow-2xl hover:shadow-primary/5 transition-all flex flex-col group cursor-pointer" @click="editOffer(pkg)">
             <div class="relative h-56 overflow-hidden">
                ...
                <div class="absolute top-4 left-4">
                   <span class="px-3 py-1 bg-white/90 backdrop-blur-md text-sm  uppercase text-primary rounded-full shadow-sm">
                      {{ pkg.tag || 'Standard' }}
                   </span>
                </div>
             ...
             <div class="p-6 flex-1 flex flex-col justify-between">
                <div class="space-y-4">
                   <p class="text-sm text-gray-400 font-medium leading-relaxed line-clamp-2">{{ pkg.description }}</p>
                   <div class="flex items-center justify-between py-4 border-t border-gray-50">
                      <div>
                         <p class="text-sm font-bold text-gray-400 uppercase">Starting From</p>
                         <p class="text-xl  text-gray-900">${{ pkg.price?.toLocaleString() || 0 }}</p>
                      </div>
                      <div class="text-right">
                         <p class="text-sm font-bold text-gray-400 uppercase">Agency Commission</p>
                         <p class="text-sm  text-emerald-500">{{ pkg.commission || 0 }}% Earn</p>
                      </div>
                   </div>
                </div>
                <div class="pt-4 flex items-center space-x-2">
                   <button class="flex-1 py-2.5 bg-gray-50 hover:bg-gray-100 text-gray-700 text-sm font-bold rounded-xl transition-all">Details</button>
                   <button class="flex-1 py-2.5 bg-primary text-white text-sm font-bold rounded-xl shadow-lg shadow-primary/20 hover:bg-black transition-all">Sell Offer</button>
                </div>
             </div>
          </div>
       </div>
    ...
    <BaseSidedrawer :show="isCreateDrawerOpen" title="New Dynamic Offer Architecture" @close="isCreateDrawerOpen = false">
       <form @submit.prevent="handleCreatePackage" class="space-y-6">
          <div class="space-y-4 pt-2 border-b border-gray-50 pb-4">
             ...
             <div class="relative group cursor-pointer border-2 border-dashed border-gray-200 rounded-xl overflow-hidden h-32 flex items-center justify-center bg-gray-50 hover:border-primary transition-colors" @click="triggerFileUpload">
                ...
                <div v-else class="text-center">
                   ...
                   <span class="text-sm font-bold text-gray-400">{{ uploading ? 'Uploading...' : 'Click to Upload Cover' }}</span>
                </div>
                <div v-if="form.image" class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                   <span class="text-sm font-bold text-white">Change Cover</span>
                </div>
          ...
          <div class="space-y-1.5">
             <label class="text-sm font-bold text-gray-400 px-1">Offer Title</label>
             <input v-model="form.title" placeholder="e.g., Ultimate Maldives Retreat" class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold outline-none focus:border-primary transition-all" />
          </div>
          
          <div class="space-y-1.5">
             <label class="text-sm font-bold text-gray-400 px-1">Captivating Description</label>
             <textarea v-model="form.description" rows="3" placeholder="Describe the holiday experience..." class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold outline-none focus:border-primary transition-all resize-none"></textarea>
          </div>

          <div class="space-y-1.5">
             <label class="text-sm font-bold text-gray-400 px-1">Offer Tag (E.g. Premium)</label>
             <input v-model="form.tag" placeholder="e.g., Best Seller" class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold outline-none focus:border-primary transition-all" />
          </div>

          <div class="grid grid-cols-2 gap-4">
             <div class="space-y-1.5">
                <label class="text-sm font-bold text-gray-400 px-1">Retail Price (USD)</label>
                <input v-model="form.price" type="number" class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold outline-none focus:border-primary transition-all" />
             </div>
             <div class="space-y-1.5">
                <label class="text-sm font-bold text-gray-400 px-1">Markup Percentage</label>
                <input v-model="form.commission" type="number" class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold outline-none focus:border-primary transition-all" />
             </div>
          </div>
          ...
          <div class="space-y-3">
             ...
             <div class="grid grid-cols-2 gap-3">
                <button type="button" v-for="srv in services" :key="srv.id" @click="toggleService(srv.id)" class="flex items-center space-x-3 p-3 rounded-xl border transition-all" :class="form.includes.includes(srv.id) ? 'border-primary bg-primary/5' : 'border-gray-50 bg-gray-50'">
                   ...
                   <span class="text-sm font-bold" :class="form.includes.includes(srv.id) ? 'text-primary' : 'text-gray-400'">{{ srv.label }}</span>
                </button>
             </div>
          </div>
          ...
          <div class="p-4 bg-emerald-50 rounded-xl border border-emerald-100 flex items-start space-x-3">
             ...
             <p class="text-sm text-emerald-700 font-bold leading-relaxed">Dynamic packages are instantly indexed for your marketplace and shared with your traveler directory.</p>
          </div>

          <button type="submit" :disabled="saving || uploading" class="w-full py-3.5 bg-primary text-white text-sm font-bold rounded-xl shadow-lg shadow-primary/20 hover:bg-black transition-all mt-4 disabled:opacity-50">
