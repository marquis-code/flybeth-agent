  ...
  <div class="bg-white rounded-[2.5rem] border border-gray-200 overflow-hidden relative w-full">
    ...
    <div class="p-8 lg:p-12">
      ...
      <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-10 pb-8 border-b border-white/10">
        <div class="space-y-1">
          <h2 class="text-lg  text-white  ">Premium Properties</h2>
          <p class="text-sm font-bold text-white/40  ">Curated stays from global hospitality networks</p>
        </div>
        ...
        <div class="flex items-center space-x-6">
          <label class="flex items-center space-x-3 cursor-pointer group">
            ...
            <span class="text-sm  text-white/60   group-hover:text-white transition-colors">Luxury Only</span>
          </label>
        </div>
      </div>
            ...
            <Transition name="slide-up">
              <div v-if="destOpen && filteredDestinations.length" class="absolute top-full left-0 right-0 mt-3 bg-white border border-gray-200 shadow-2xl rounded-[2rem] z-[9999] max-h-[400px] overflow-y-auto p-3 animate-in fade-in slide-in-from-top-4">
                ...
                >
                  ...
                  <ChevronDownIcon class="h-4 w-4 text-gray-800 -rotate-90" />
                </button>
              </div>
            </Transition>
          </div>
          ...
          <div class="lg:col-span-5 relative" ref="guestPopoverRef">
            ...
            >
              <div class="flex items-center space-x-3">
                ...
                <div class="text-left">
                  <p class="text-sm  text-primary   mb-0.5" v-if="totalGuests > 0">Travelers</p>
                  <p class="text-sm font-bold text-gray-900">{{ guests.adults + guests.children }} Guests, {{ guests.rooms }} Room</p>
                </div>
              </div>
              <ChevronDownIcon class="h-4 w-4 text-gray-800 transition-transform" :class="showGuests ? 'rotate-180' : ''" />
            </button>
            <!-- Guests Dropdown -->
            <Transition name="slide-up">
              <div v-if="showGuests" class="absolute top-full right-0 left-0 lg:left-auto mt-3 w-full lg:w-80 bg-white border border-gray-200 shadow-2xl rounded-[2rem] z-[9999] p-8 space-y-8 animate-in fade-in slide-in-from-top-4">
                <div v-for="cat in guestCategories" :key="cat.id" class="flex items-center justify-between">
                  ...
                  <div class="flex items-center space-x-4">
                    ...
                    <span class="text-sm  w-6 text-center">{{ guests[cat.id as keyof typeof guests] }}</span>
    ...
    <Transition name="fade">
      <div v-if="stayResults.length > 0" class="border-t border-gray-200 bg-gray-50/50 p-8 lg:p-12">
        <div class="flex items-center justify-between mb-10">
          <h3 class="text-sm  text-gray-900  ">Luxury Results</h3>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          >
            <div class="h-60 bg-gray-100 relative overflow-hidden">
              ...
              <div class="absolute top-4 right-4 z-10 px-3 py-1 bg-white/90 backdrop-blur-md text-sm  text-secondary   rounded-full shadow-sm">Special Offer</div>
              <div class="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent"></div>
              <div class="absolute bottom-4 left-6 text-white">
                <div class="flex items-center space-x-1">
                  <StarIcon class="h-3 w-3 text-yellow-400 fill-yellow-400" />
                  <span class="text-sm ">{{ stay.rating || '4.9' }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="p-8 space-y-4 flex-1 flex flex-col">
              <div class="space-y-2">
                <h4 class="text-sm  text-gray-900  group-hover:text-primary transition-colors leading-tight ">{{ stay.name }}</h4>
                <p class="text-sm  text-gray-800   flex items-center space-x-1">
              ...
              <div class="pt-6 border-t border-gray-100 flex items-end justify-between mt-auto">
                <div class="space-y-0.5">
                  <p class="text-sm  text-gray-800  ">Starting at</p>
                  <p class="text-lg  text-primary-dark er">${{ stay.price }}<span class="text-sm font-medium text-gray-800 ml-1">/night</span></p>
                </div>
                <BaseButton variant="primary" class="rounded-xl px-6 h-12 text-sm   active:scale-95 transition-all  shadow-lg shadow-primary/10">Reserve</BaseButton>
              </div>
            </div>
          </div>
        </div>
