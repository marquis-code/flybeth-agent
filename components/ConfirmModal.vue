    ...
    <Transition name="modal-backdrop">
      <div v-if="visible" class="fixed inset-0 z-[200] flex items-center justify-center p-4" @click.self="onBackdropClick">
        ...
        <Transition name="modal-content" appear>
          <div v-if="visible" class="relative bg-white rounded-[2rem] shadow-[0_40px_120px_-20px_rgba(31,38,135,0.15)] max-w-md w-full overflow-hidden">
            <!-- Top Accent -->
            ...
            <div class="p-10 text-center">
              ...
              <!-- Message -->
              <p class="text-sm text-neutral-500 font-bold leading-relaxed mb-10 animate-text-up-delay">
                {{ message }}
              </p>
...
const accentClass = computed(() => {
  const map: Record<string, string> = {
    ...
    logout: 'bg-gradient-to-r from-primary to-primary-dark',
    info: 'bg-gradient-to-r from-primary to-sky-400',
...
const ringClass = computed(() => {
  const map: Record<string, string> = {
    ...
    logout: 'border-2 border-primary/20',
...
const confirmBtnClass = computed(() => {
  const map: Record<string, string> = {
    ...
    logout: 'bg-primary-dark shadow-primary/30 hover:shadow-primary/40',
    info: 'bg-primary shadow-primary/30 hover:shadow-primary/40',
...
@keyframes icon-entrance { from { transform: scale(0) rotate(-30deg); opacity: 0; } to { transform: scale(1) rotate(0deg); opacity: 1; } }
@keyframes icon-pop { 0% { transform: scale(0); } 60% { transform: scale(1.2); } 100% { transform: scale(1); } }
@keyframes icon-ring { 0% { transform: scale(0.8); opacity: 1; } 100% { transform: scale(1.5); opacity: 0; } }
@keyframes text-up { from { opacity: 0; transform: translateY(15px); } to { opacity: 1; transform: translateY(0); } }
