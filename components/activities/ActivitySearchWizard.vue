  ...
  <div class="bg-white rounded-[2.5rem] border border-gray-200 relative w-full shadow-sm">
    ...
    <div class="p-8 lg:p-12">
      ...
      <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-10 pb-8 border-b border-gray-100">
        <div class="space-y-1">
          <h2 class="text-lg  text-gray-900  ">Global Experiences</h2>
          <p class="text-sm font-medium text-gray-800  ">Curated tours, attractions & local secrets</p>
        </div>
      </div>
      ...
      <div class="mt-12 flex flex-col sm:flex-row sm:items-center justify-between gap-8 pt-8 border-t border-gray-100">
        <div class="flex -space-x-3 overflow-hidden p-1">
          ...
          <div class="flex items-center justify-center h-8 px-3 rounded-full bg-gray-50 ring-2 ring-white text-sm  text-gray-800">10k+ booked this week</div>
        </div>
    ...
    <Transition name="fade">
      <div v-if="activityResults.length > 0" class="border-t border-gray-200 bg-gray-50/50 p-8 lg:p-12">
        <div class="flex items-center justify-between mb-10">
          <h3 class="text-sm  text-gray-900  ">Top Recommended</h3>
          <div class="flex items-center space-x-4">
             <span class="text-sm  text-gray-800  ">Sort by Popularity</span>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          >
            <!-- Badge -->
            <div class="absolute top-4 left-4 z-10 px-3 py-1 bg-white/90 backdrop-blur-md text-sm  text-primary   rounded-full shadow-sm">{{ act.category }}</div>
            ...
            <div class="h-56 bg-gray-100 relative overflow-hidden group-hover:h-48 transition-all duration-700">
              ...
              <div v-else class="w-full h-full flex items-center justify-center">
                <SparklesIcon class="h-12 w-12 text-gray-500" />
              </div>
              ...
              <div class="space-y-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center space-x-1">
                    ...
                    <span class="text-sm  text-gray-900">{{ act.rating }}</span>
                    <span class="text-sm font-medium text-gray-800">({{ act.reviews }} reviews)</span>
                  </div>
                  <span class="text-sm  text-gray-800  ">{{ act.duration }}</span>
                </div>
                <h4 class="text-sm  text-gray-900  group-hover:text-primary transition-colors leading-tight ">{{ act.name }}</h4>
              </div>

              <div class="pt-6 border-t border-gray-100 flex items-end justify-between mt-auto">
                <div class="space-y-0.5">
                  <p class="text-sm  text-gray-800  ">From</p>
                  <p class="text-sm  text-primary-dark">${{ act.price }}<span class="text-sm font-medium text-gray-800 ml-1">/pp</span></p>
                </div>
                <BaseButton variant="primary" class="rounded-xl px-6 h-10 text-sm   active:scale-95 transition-all">VIEW PLAN</BaseButton>
              </div>
            </div>
          </div>
        </div>
