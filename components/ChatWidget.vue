...
const initializeChatRoom = async () => {
  try {
    const room = await findAdminAndStartChat(!isLoggedIn.value ? guestInfo.value : undefined)
...
onMounted(() => {
  ...
  const messageHandler = (e: any) => {
    ...
    if (activeRoom.value && (msg.room === activeRoom.value._id || msg.room?._id === activeRoom.value._id)) {
       // Check if this message is a server confirmation of one of our temp messages
...
<template>
  <div class="fixed bottom-6 right-6 z-[9999]">
    ...
    <button 
      @click="toggleChat"
      class="h-14 w-14 rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all duration-300 relative"
      :class="isOpen 
    ...
    >
      ...
      <div v-if="!isOpen && unreadCount === 0" class="absolute -top-0.5 -right-0.5 h-3.5 w-3.5 bg-secondary rounded-full border-2 border-white">
        <div class="absolute inset-0 bg-secondary-light rounded-full animate-ping opacity-75"></div>
      </div>
    </button>
    ...
    <Transition name="slide-up">
      <div v-if="isOpen" class="absolute bottom-20 right-0 w-[400px] max-w-[calc(100vw-32px)] max-h-[600px] h-[80vh] bg-white rounded-2xl shadow-2xl shadow-black/20 border border-gray-100 flex flex-col overflow-hidden origin-bottom-right">
        <!-- Header -->
        <div class="p-5 bg-gradient-to-r from-primary via-primary-dark to-[#050B4A] text-white shrink-0">
        ...
        <div v-if="isIdentifying" class="flex-1 p-6 flex flex-col justify-center space-y-6">
           ...
           <button @click="submitIdentification" :disabled="!guestInfo.name.trim() || !guestInfo.email.trim()" class="w-full py-3.5 bg-gradient-to-r from-primary to-primary-dark text-white rounded-xl font-bold text-sm shadow-lg shadow-primary/20 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-30 transition-all">Start Chatting →</button>
        </div>
        ...
        <div v-else ref="scrollContainer" class="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50/80">
          ...
          <template v-else>
            <div v-for="(msg, idx) in messages" :key="msg._id || idx" class="flex flex-col" :class="isMyMessage(msg) ? 'items-end' : 'items-start'">
...
.slide-up-leave-to { opacity: 0; transform: translateY(16px) scale(0.96); }
