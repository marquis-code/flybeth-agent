...
function rangeWrapClass(iso: string): string {
  ...
  if (s && end && iso < end) return 'bg-gray-100 rounded-l-full cursor-pointer'
  if ((e || (end && iso === end && !s)) && startDate.value && iso > startDate.value) return 'bg-gray-100 rounded-r-full cursor-pointer'
  if (inR) return 'bg-gray-100 cursor-pointer'
...
function updatePosition() {
  ...
  panelStyle.value = { top:  `${rect.bottom + 8}px`, left: `${Math.max(8, left)}px` }
}
