<template>
  <header 
    :class="[
      ...
      isScrolled 
        ? 'bg-white/80 backdrop-blur-2xl border-b border-black/5 py-3 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.1)]' 
        : 'bg-transparent py-6'
    ]"
  >
    <div class="px-8 lg:px-10">
      <nav class="flex items-center justify-between">
        ...
        <div class="hidden lg:flex items-center space-x-2">
          ...
          >
            {{ item.name }}
            <div class="absolute bottom-0 left-5 right-5 h-[1.5px] bg-primary scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></div>
          </NuxtLink>
        </div>
          ...
          <template v-if="user">
            ...
            >
              <div class="flex flex-col items-end mr-4">
                <span class="text-sm font-bold text-primary-dark leading-tight">{{ user.firstName }} {{ user.lastName }}</span>
                <span class="text-sm  text-secondary   opacity-80">
              ...
              <div class="w-9 h-9 rounded-full bg-gradient-to-tr from-secondary/20 to-primary/20 flex items-center justify-center border border-white shadow-sm overflow-hidden transition-transform group-hover:scale-105">
                 ...
                 <span v-else class="text-sm font-bold text-secondary">{{ user.firstName?.charAt(0) }}</span>
              </div>
              <ChevronDownIcon 
            ...
            >
              ...
              >
                <div class="p-5 border-b border-gray-50 flex items-center space-x-4">
                  <div class="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary font-bold text-sm">
                    {{ user.firstName?.charAt(0) }}
                  </div>
                  <div class="flex flex-col">
                    <p class="text-[14px] font-bold text-gray-900 leading-tight">{{ user.firstName }} {{ user.lastName }}</p>
                    <p class="text-sm text-neutral-400 font-medium truncate">{{ user.email }}</p>
                  </div>
                </div>
                ...
                <div class="p-2">
                  ...
                  >
                    <div class="w-10 h-10 rounded-xl bg-gray-50 group-hover:bg-white flex items-center justify-center transition-colors">
                      <PresentationChartLineIcon class="h-5 w-5 text-gray-800 group-hover:text-primary" />
                    </div>
                    <span class="text-sm font-bold text-gray-800 group-hover:text-primary">Agent Dashboard</span>
                  </button>
                  ...
                  >
                    <div class="w-10 h-10 rounded-xl bg-gray-50 group-hover:bg-white flex items-center justify-center transition-colors">
                      <UserCircleIcon class="h-5 w-5 text-gray-800 group-hover:text-secondary" />
                    </div>
                    <span class="text-sm font-bold text-gray-800 group-hover:text-secondary">Profile Settings</span>
                  </button>
                </div>
        ...
        <div class="hidden lg:flex items-center space-x-8">
          ...
          <template v-else>
            ...
            <BaseButton 
              ...
              variant="primary" 
              size="md" 
              class="px-8 py-2.5 font-bold text-[13px] shadow-lg shadow-primary/20"
            >
              Get Started
            </BaseButton>
          </template>
      ...
      <div v-if="isMobileMenuOpen" class="lg:hidden absolute top-[calc(100%+12px)] left-0 right-0 bg-white border border-white/20 rounded-[32px] shadow-[0_24px_48px_-12px_rgba(0,0,0,0.15)] p-8 space-y-8 overflow-hidden z-[60]">
        ...
        <div class="flex flex-col space-y-1 relative z-10">
          <NuxtLink 
            ...
            class="text-sm font-bold text-primary-dark p-4 rounded-2xl hover:bg-secondary/5 transition-all flex items-center justify-between group"
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-1 gap-2">
                ...
                >
                  ...
                  <span class="text-sm font-bold text-gray-800  ">Dashboard</span>
                </button>
                ...
                >
                  ...
                  <span class="text-sm font-bold text-gray-800  ">Profile Settings</span>
                </button>
              </div>
            </div>
          </template>
...
const navItems = [
  ...
  { name: 'Partners', path: '/travel-partners' },
  { name: 'Team', path: '/leadership-team' },
...
// On non-homepage pages, always show scrolled (dark text) state since those pages have white backgrounds
const handleScroll = () => {
  if (route.path !== '/') {
